import { initialized, kepler } from "@spruceid/sprucekit-wasm";
import {
  ConfigOverrides,
...
export class KeplerStorage implements IStorage, IKepler {
  ...
  public async afterConnect(
    sprucekit: UserAuthorizationConnected
    ...
    this.domain = sprucekit.config.siweConfig?.domain;
    return {};
  }
  ...
  public async generateKeplerSession(
    spruceKitSession: SpruceKitClientSession
  ): Promise<SpruceKitClientSession> {
    return await Promise.resolve({
      jwk: JSON.parse(spruceKitSession.sessionKey),
      orbitId: this.orbitId,
      service: "kv",
      siwe: spruceKitSession.siwe,
      signature: spruceKitSession.signature,
      verificationMethod: new SiweMessage(spruceKitSession.siwe).uri,
    })
      .then(JSON.stringify)
  ...
  public async afterSignIn(
    ...
    const session = await this.generateKeplerSession(spruceKitSession);
    ...
    try {
      ...
      if (this.autoCreateNewOrbit === true) {
        await this.hostOrbit(spruceKitSession);
        return;
      }
    }
  ...
  public async activateSession(
    ...
    try {
      if (!spruceKitSession) {
        ({ session: spruceKitSession } = this.userAuth);
      }
  ...
  public async hostOrbit(
    ...
    await this.activateSession(spruceKitSession, () => {
      throw new Error(
